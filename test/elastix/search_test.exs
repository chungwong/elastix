defmodule Elastix.SearchTest do
  ...
  test "make_path should make path from id and url" do
    path = Search.make_path(@test_index, ttl: "1d", timeout: 123)
    assert path == "/#{@test_index}/_search?ttl=1d&timeout=123"
  end
  ...
  test "make_path should make path that can interchange api type" do
    ...
    assert path == "/#{@test_index}/_count?ttl=1d&timeout=123"
  end

  test "search should return with status 200" do
    Document.index(@test_url, @test_index, 1, @document_data, refresh: true)
  ...
  test "search accepts a list of indexes" do
    Document.index(@test_url, @test_index, 1, @document_data, refresh: true)
    Document.index(@test_url, @test_index_2, 1, @document_data, refresh: true)
  ...
  test "search accepts a list of requests" do
    Document.index(@test_url, @test_index, 1, @document_data, refresh: true)
    Document.index(@test_url, @test_index, 2, @document_data, refresh: true)
